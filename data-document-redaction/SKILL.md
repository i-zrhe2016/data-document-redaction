...
description: "处理数据集和文档中的个人信息、凭据与商业机密，并在脱敏完成后独立二次审查交付副本，输出不含原值的审查报告。适用于 CSV、JSON、SQL、表格、PDF、Word、PowerPoint、图片、扫描件和日志；不用于恢复或绕过脱敏、普通校对或法律认证。"
---
...
本 skill 有两个相关但不能混为一谈的模式：

- **生成脱敏副本**：识别敏感内容，按目标用途做最小充分变换，并完成一次独立验证。
- **文档完成后的二次审查**：对已经生成的候选副本做交付前、只读、独立复核；目标是发现首轮漏检、隐藏表面残留、不可验证的内容和效用损坏，判断是否可以在声明的范围内交付。二次审查不是重新脱敏、普通文字校对、恢复原文、绝对匿名化保证或法律/合规认证。

当用户说“完成后复核”“交付前审查”“二次审查”“独立验证”或提供了已有的脱敏文件与首轮处理记录时，使用二次审查模式。审查应以候选输出、需求/验收标准和首轮报告为主要输入；如需与原件比较，只在获授权的本地环境中计算哈希、结构或统计差异，不把原文、原值或映射表载入上下文，也不在审查阶段覆盖候选文件。

无论使用哪种模式，完成时都必须输出审查报告；二次审查模式必须额外输出独立复核结论，且不能把首轮工具的“成功”标记当作二次证据。
...
7. **独立复核**：使用与生成工具不同的读取路径（例如另一 PDF 阅读器/解析器、文本抽取、搜索、渲染截图、元数据检查）验证“敏感值不存在”及“业务效用仍满足”。对 PDF 至少做选择、搜索、复制、文本抽取、元数据、书签、注释、表单、附件和 OCR 检查。
8. **交付报告**：按 [output-report.md](references/output-report.md) 输出变换清单、检测计数、结构校验、复核结果、残余风险、假设和未覆盖面；永远不输出原值、映射表或秘密。

## 文档完成后的二次审查工作流

二次审查应在首轮生成结束后重新启动一条证据链，详细清单见 [secondary-review.md](references/secondary-review.md)：

1. **锁定声明范围**：读取交付目的、接收者、允许保留的效用、首轮报告和禁止实体/词表；缺少验收标准时标记假设，不擅自扩大“安全”结论。
2. **确认候选副本**：记录文件类型、大小、SHA-256、生成时间和是否为新路径；确认原件未被覆盖。不能确认来源或版本时为 `needs_review`。
3. **独立重读**：改用不同解析器、读取路径或进程重新提取文本/结构；重新扫描原值、别名、昵称、缩写、分词变体、编码值和秘密模式。只输出类型、位置类别和计数。
4. **逐表面复核**：按 [document-surfaces.md](references/document-surfaces.md) 检查可见内容、隐藏文本、OCR、批注/修订、元数据、链接、附件、嵌入对象、文件名和历史；自动检测只是辅助，logo、图像、重复页眉、自由文本和扫描页必须有视觉或人工覆盖证据。
5. **复核效用与完整性**：按声明的用途检查可读性、版式、页数、schema、行数、外键/唯一约束、公式、分布、偏斜和异常值；任何未声明的损坏都应记录为发现项。
6. **做出范围化结论**：`pass` 仅表示声明范围内的检查均通过；有残留、漏检或未覆盖表面用 `needs_review`；加密、损坏、低置信度 OCR 或关键表面无法读取用 `blocked`。给每个发现分配严重级别、位置类别、证据和修复建议，不写原值。
7. **输出审查报告**：必须按 [output-report.md](references/output-report.md) 生成 Markdown 报告；如调用方需要机器读取，再附同内容的 JSON。报告应能让另一位审阅者复现检查范围与结论，而不会成为敏感数据副本。
...
| 已完成的候选副本、交付前复核 | 使用二次审查模式，只读重建证据链 | 独立解析/扫描、视觉覆盖、完整性与效用、审查报告 |
...
- 文档完成后的二次审查流程：[secondary-review.md](references/secondary-review.md)
